...
class _RegisterPageState extends State<RegisterPage> {
  ...
  bool _responsavelConsentiu = false;
  ...
  void _register() async {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        // Também criar usuário no banco (AppDatabase)
        // e persistir série em SharedPreferences
        try {
          // Buscar por username no DB (case-insensitive)
          final existing =
              await AppDatabase.instance.getUserByUsername(
          ...
          if (existing == null) {
            ...
            Logger.d(
              'Usuário criado no DB com ID: ${created.id}',
            );
          } else {
            created = existing;

            Logger.d(
              'Usuário já existente no DB: '
              '${existing.username} (ID: ${existing.id})',
            );
          }
          ...
          try {
            ...
            if (created.grade != null) {
              await prefs.setString(
                'usuario_grade',
                _canonicalGrade(created.grade),
              );
            }
          } catch (e) {
            Logger.d(
              'Erro ao salvar SharedPreferences após cadastro: $e',
            );
          }
        } catch (e) {
          Logger.d(
            'Erro ao criar usuário no banco: $e',
          );
        }
  ...
  String? _resolveClassGroupName() {
    // Se há seleção de turma cadastrada, usa o nome;
    // senão usa o texto livre, se existir
    if (_selectedClassGroupId != null) {
      ...
      return found.name.isNotEmpty
          ? found.name
          : null;
    }

    return _classGroupController.text.trim().isEmpty
        ? null
        : _classGroupController.text.trim();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final availableGroups =
        (_selectedSchool != null && _selectedSerie != null)
            ? _classService.getBySchoolAndGrade(
                _selectedSchool!.id,
                _selectedSerie!,
              )
            : const <ClassGroup>[];
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration:
                        ...
                        labelText:
                            ...
                            '(apelido opcional)',
                        helperText:
                            'Se não informar, usaremos '
                            'seu primeiro nome e a inicial '
                            'do sobrenome.',
                        prefixIcon:
                    ...
                    if (availableGroups.isNotEmpty) ...[
                      ...
                      TextFormField(
                        controller:
                            _classGroupController,
                        decoration:
                            const InputDecoration(
                          labelText:
                              'Turma (ex.: 5ºA) - opcional',
                          prefixIcon:
                              Icon(Icons.class_),
                          border:
                              OutlineInputBorder(),
                    ...
                    Container(
                      ...
                      child: CheckboxListTile(
                        ...
                        subtitle: const Text(
                          'Declaro que sou responsável pelo '
                          'aluno e autorizo o tratamento dos '
                          'dados necessários para sua '
                          'participação nas atividades '
                          'educacionais do SQEducaPlay.',
                        ),
                      ),
                    ),
